...
- no log page shows an empty body or placeholder text without an "Under Construction" banner. **Done:** every log deck that still renders filler or an empty body carries a `🚧 Under Construction` / `🚧 În Construcție` banner directly beneath the category header — 24 decks at the time of writing. The other 22 have since been written and their banners were pruned, because a notice left on finished prose is the same lie with its sign flipped: it tells a reader that real content is placeholder and sends them away from a completed page. Styling is Module 32 in `en/style.css` / `ro/style.css`, with narrow-deck metrics in Module 00.2 §B6. `tools/add-uc-banner.py` writes banners in (`--apply`) and takes them back out when a deck is finished (`--prune`); `tools/check-uc-banner.py` is the regression gate and runs in `.github/workflows/lint.yml`. The gate fails in BOTH directions — a deck showing filler with no banner (NAKED) and a written deck still carrying one (STALE) — and it judges empty bodies as well as filler, so a blank deck cannot pass by containing no "lorem". Because `--prune` edits pages other work is actively writing, `tools/test-uc-prune-safety.py` proves removal deletes banner lines only, and `tools/test-uc-prune-selftest.py` proves that proof can still fail by injecting three real faults; both run in CI.
...
**Affected files:** `JOURNAL-EN.md`, `JOURNAL-RO.md`, `docs/drafts/day22-draft-en.md`, `docs/drafts/day22-draft-ro.md`
