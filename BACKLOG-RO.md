...
- nicio pagină de jurnal nu arată un corp gol sau text placeholder fără un banner „În construcție". **Realizat:** fiecare punte de jurnal care încă afișează text de umplutură sau un corp gol poartă un banner `🚧 Under Construction` / `🚧 În Construcție` direct sub antetul de categorie — 24 de punți în momentul redactării. Celelalte 22 au fost între timp scrise, iar bannerele lor au fost înlăturate: o notificare lăsată peste proză terminată este aceeași minciună cu semnul schimbat — îi spune cititorului că un conținut real este text de umplutură și îl alungă de la o pagină finalizată. Stilizarea este Modulul 32 din `en/style.css` / `ro/style.css`, cu metricile pentru punți înguste în Modulul 00.2 §B6. `tools/add-uc-banner.py` scrie bannerele (`--apply`) și le retrage atunci când o punte este terminată (`--prune`); `tools/check-uc-banner.py` este poarta de regresie și rulează în `.github/workflows/lint.yml`. Poarta cade în AMBELE direcții — o punte cu text de umplutură fără banner (NAKED) și o punte scrisă care încă poartă unul (STALE) — și verifică și corpurile goale, nu doar textul de umplutură, astfel încât o punte goală nu poate trece pentru că nu conține „lorem". Deoarece `--prune` editează pagini la care se lucrează în paralel, `tools/test-uc-prune-safety.py` demonstrează că înlăturarea șterge doar liniile bannerului, iar `tools/test-uc-prune-selftest.py` demonstrează că acea probă poate încă să eșueze, injectând trei defecte reale; ambele rulează în CI.
